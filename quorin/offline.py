...
:mod:`quorin.wal_consumer`. Each ``flush()`` writes one Parquet file per
...
from quorin._internal.arrow_schema import _arrow_plan_for, _ArrowPlan
...
class ParquetDatasetStore:
    """Single-coroutine async offline writer for the Quorin WAL consumer.

    **Concurrency contract:** This class is NOT safe for concurrent
    ``append``/``flush`` from multiple asyncio tasks. The Step 10
    :class:`quorin.wal_consumer.WALConsumer` guarantees single-coroutine
