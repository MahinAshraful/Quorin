...
Carves a Quorin shared-memory segment (allocated by :mod:`quorin.shm` in
...
def _slot_field_offset(name: str) -> int:
    ...
    so the kernel in :mod:`quorin.assembly` (Step 8) doesn't carry magic
...
# ---------------------------------------------------------------------------
# Internal alignment helper. Mirrored from quorin.schema._align_up so the
...
# create/open time and stored on the :class:`quorin.shm.Segment` instance.
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class SegmentLayout:
    """All derived offsets and sizes for a multi-entity Quorin segment."""
    ...
    # Length of the float32 vector returned by quorin.serving.assemble.
    # Sum of element_count across all fields.
    total_element_count: int

    # Flat parallel arrays derived from assembly_table — used by Step 5's
    # Numba kernel (quorin.assembly._assemble_core). Pre-cast to int64/uint8
...
def total_segment_size(
    ...
    *,
    ...
    Step 2's :class:`quorin.shm.SegmentRegistry` calls this to size the POSIX
    shm allocation.
    """
...
def compute_layout_from_segment(
    ...
    Used by :meth:`quorin.shm.SegmentRegistry.open_current` so that opening
...
def initialize_segment_regions(buf: memoryview, layout: SegmentLayout) -> None:
    ...
    Called by :class:`quorin.shm.SegmentRegistry.create` once the segment
...
# ---------------------------------------------------------------------------
# Public helper — kwargs-style row packing for the synchronous insert path.
# ---------------------------------------------------------------------------
...
def pack_row(schema: type[FeatureSchema], /, **fields: Any) -> bytes:
    ...
    Args:
        ...
        **fields: One keyword argument per declared field. Scalar values are
            ...
            declared dtype via ``np.asarray(value, dtype=...)``.

    Returns:
        Exactly ``row_size(schema)`` bytes — pass directly to
        :func:`insert` as the ``row_bytes`` argument.

    Raises:
        ValueError: missing field, unknown extra field, or shape mismatch.

    For high-throughput production writes prefer
    :class:`quorin.wal.WALProducer`, which validates via pydantic and routes
    through the Redis-Stream WAL with crash-safety semantics. ``pack_row`` is
    for the synchronous insert path (testing, hydration, demos).
    """
    declared = {f.name for f in schema.fields}
    provided = set(fields.keys())
    if missing := declared - provided:
        raise ValueError(f"pack_row missing fields: {sorted(missing)}")
    if extra := provided - declared:
        raise ValueError(f"pack_row unknown fields: {sorted(extra)}")

    rs = row_size(schema)
    out = bytearray(rs)
    table = compute_assembly_table(schema)  # declaration order
    ...
    for i, f in enumerate(schema.fields):
        ...
        src = np.asarray(fields[f.name], dtype=expected_dtype).reshape(-1)
        if src.size != elem_cnt:
            raise ValueError(
                f"pack_row[{f.name!r}]: element_count {src.size} != expected {elem_cnt}"
            )
        view = np.frombuffer(out, dtype=expected_dtype, count=elem_cnt, offset=byte_off)
        np.copyto(view, src)
