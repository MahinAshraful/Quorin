...
Step 10's :class:`quorin.wal_consumer.WALConsumer` decodes the producer's
...
``bytearray`` matching ``quorin.schema.row_size(schema)`` so that
``quorin.layout.insert(seg, entity_id, row_buffer)`` lands the row at the
...
1. **Plan walks** :func:`quorin.schema.compute_row_offset_table` —
   name_hash-sorted, row-relative offsets. Walking
   :func:`quorin.schema.compute_assembly_table` (declaration order)
...
2. **One consolidated** :class:`struct.Struct` **for every scalar field**
   ...
   pads, the consolidator would only catch field 0 (because Quorin
   aligns every field start to 64 bytes; see
   :func:`quorin.schema._field_byte_offsets`) and dump 199 fields into
...
4. **Inter-field padding bytes are not zeroed.** The caller-supplied
   ...
   :func:`quorin.serving.assemble` walks the offset table — it reads
   only ``(byte_offset, byte_count)`` extents and never touches padding
   bytes. ``quorin.layout.insert`` copies the full row into shm
...
class _PackPlan:
    ...
    row_size: int
    """``quorin.schema.row_size(schema)``. Validated against ``len(out)`` at
    every call."""
...
def _build_plan(schema: type[FeatureSchema]) -> _PackPlan:
    ...
    shape_by_hash: dict[int, tuple[int, ...]] = {
        # Lazy import: quorin.schema._hash_name is private; we already
