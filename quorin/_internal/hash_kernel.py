...
Mirror image of :mod:`quorin._internal.hash_id`'s ``hash_entity_id`` —
...
--------------------------------
Lives at ``quorin/_internal/hash_kernel.py`` (NOT in
``quorin/_internal/hash_id.py``) because ``hash_id`` is imported by
``quorin.layout``, ``quorin.serving``, etc. — non-Numba modules.
``hash_kernel`` only gets pulled by Numba-aware modules
(``quorin._internal.lookup_kernel``, eventually
``quorin.assembly.assemble_batch``).
...
def prewarm() -> None:
    ...
    NOT auto-run at module import. :func:`quorin.assembly.prewarm`
