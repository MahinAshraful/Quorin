...
Mirror image of Step 8's :func:`quorin.assembly._assemble_batch_core` —
...
``quorin._internal.insert_kernel`` (the bulk path; Step 13 hydration)
...
- **fastmath=False** is non-negotiable across all Quorin Numba
...
from quorin._internal.hash_id import hash_entity_id_bytes
...
from quorin.schema import DTYPE_TO_NUMPY, FeatureField, FeatureSchema
...
def prewarm() -> None:
    ...
    Mirrors :func:`quorin.assembly.prewarm`. First call to the kernel
...
# ---------------------------------------------------------------------------
# Public wrapper + column preparation. Lives here (not in quorin.layout) to
# respect invariant #11: quorin.layout has many non-Numba callers (lookup,
# per-row insert, the WAL consumer's `_apply` hot path) and shouldn't pull
# Numba into its import surface. Mirrors the quorin.assembly.assemble_batch
