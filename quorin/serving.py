"""Serving path — assemble a feature vector from a Quorin segment.
...
The :class:`quorin.layout.SegmentLayout` carries two parallel structures:
``row_offset_table`` (hash-sorted, used by :func:`quorin.layout.lookup`) and
...
from quorin.schema import DTYPE_TO_NUMPY, DType
...
class EntityNotFoundError(RuntimeError):
    ...
    Inherits :class:`RuntimeError` to match the existing quorin convention
    ...
    *,
    ...
    Raises:
        ValueError: if ``entity_id`` is empty (delegated to
            :func:`quorin.layout.lookup`), or if ``out`` does not match the
