...
Reads from the Redis Stream ``quorin:wal`` produced by Step 9's
:class:`quorin.wal.WALProducer`, applies each message to the online
store (shared memory via :func:`quorin.layout.insert`) and to an
...
``quorin:processed:{msg_id}`` side table that ``WALProducer.write_sync``
polls.

Two events the consumer signals are deliberately decoupled:

- ``SET quorin:processed:{msg_id} EX 86400`` fires immediately after
...
3. Borrowed segments — constructor accepts pre-opened
   :class:`quorin.shm.Segment` instances. The user owns segment
...
8. Distributed consumer-name lock at startup
   (``quorin:consumer:lock:{group}:{name}``, 60s TTL renewed each
...
from quorin.schema import FeatureSchema, row_size
...
DEFAULT_GROUP_NAME = "quorin_consumers"
...
CONSUMER_LOCK_PREFIX = b"quorin:consumer:lock:"
...
# Single source of truth; `quorin.hydration` imports the constant here.
...
class ConsumerNameInUseError(RuntimeError):
    ...
    holding ``quorin:consumer:lock:{group}:{name}`` in Redis. Two consumers
...
class WALConsumer:
    ...
    Constructor:
        ...
        registry: The :class:`quorin.shm.SegmentRegistry` that opened
    ...
    def __init__(
        ...
        *,
        ...
        # Step 15 pause-key prefix: quorin:upgrade:pause:{safe_class_name}.
        ...
        # convention).
        self._pause_key_prefix_b = b"quorin:upgrade:pause:"
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            flush_task = asyncio.create_task(self._flush_loop(), name="quorin-wal-flush")
            try:
    ...
    async def _run_one_iter(self) -> None:
        ...
        if now - self._liveness_last_refresh >= LIVENESS_REFRESH_INTERVAL_SECONDS:
            try:
                ...
                # on `quorin_wal_consumer_liveness_refresh_failures_total{outcome="error"}`
                # AND on `quorin_wal_consumer_liveness_age_seconds > 30`.
    ...
    async def _check_upgrade_pause_and_reopen(self, _msgs: object) -> bool:  # noqa: PLR0912 (sequential reopen branches)
        ...
        # signal (`quorin:processed:{msg_id}`) is honored for in-flight
    ...
    async def _acquire_consumer_lock(self) -> None:
        """SET quorin:consumer:lock:{group}:{name} <pid> EX 60 NX."""
