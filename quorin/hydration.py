...
from quorin._internal.insert_kernel import insert_many, prewarm
from quorin.logging import get_logger
from quorin.metrics import hydration_entity_count, hydration_seconds
...
from quorin.wal_consumer import KEY_WAL_CONSUMER_LIVENESS
...
class HydrationConflictError(HydrationError):
    ...
    so it does NOT tick :data:`quorin_hydration_seconds`.
    """
...
def hydrate(  # noqa: PLR0915 — orchestration logic is intentionally linear for readability
    ...
    *,
    ...
    Metrics
    -------
    ``quorin_hydration_seconds{outcome=ok|err}``
        ...
        ``rate(quorin_hydration_seconds_count{outcome="err"}[5m])``
    ...
    # `cast` mirrors the pattern in quorin.shm.SegmentRegistry.open_current —
    ...
    if current is not None:
        ...
        raise HydrationConflictError(
            ...
            f"`redis-cli DEL quorin:schema:{schema.__name__}:current` "
    ...
    try:
        ...
        # counted in `quorin_hydration_seconds` at all. See docstring
        # "Metrics" section.
...
def _force_drop_orphan(
    ...
    quorin:cleanup_queue → Step 14 watchdog drain``). Justification:
    ...
    4. **DO NOT** "fix" this to call ``registry.close()``. ``registry.close()``
       would push our own segment name into ``quorin:cleanup_queue``
