...
dtype=np.float32)``) from :func:`quorin.serving.assemble` and
:func:`quorin.assembly.assemble`. A :class:`BufferPool` is constructed once
...
from quorin.schema import FeatureSchema, total_element_count
...
class BufferPool:
    ...
    def schema_name(self) -> str:
        ...
        Used as the ``schema`` label on :data:`quorin.metrics.pool_miss_total`.
        """
...
class BatchBufferPool:
    ...
    **Default ``zero_on_return=False`` safety contract**
    ...
    Safe for :func:`quorin.assembly.assemble_batch` because the kernel
