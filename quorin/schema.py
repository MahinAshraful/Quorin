...
A Quorin schema is a declarative Python class that names the typed fields of
...
"""Bytes reserved at the start of every Quorin segment for magic + version + CRC32."""
...
"""DType IntEnum -> NumPy dtype object. ``quorin.serving.assemble`` uses this
...
class FeatureSchema:
    """Base class for all Quorin schemas.
...
def total_segment_size(schema: type[FeatureSchema]) -> int:
    ...
    .. note::
        ...
        original Step 2 placeholder. The real Quorin segment is multi-entity
        and uses :func:`quorin.layout.total_segment_size` (which takes
...
def total_element_count(schema: type[FeatureSchema]) -> int:
    ...
    returned by :func:`quorin.serving.assemble`.
    """
