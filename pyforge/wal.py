...
Pyforge writes are async-by-default: the caller invokes
...
5. **NaN/Inf accepted.** Float fields use ``allow_inf_nan=True`` in the
   ...
   :mod:`pyforge._internal.pydantic_factory` and ADR-008 §5.
...
from pyforge._internal.pydantic_factory import field_order_for, pydantic_model_for
...
DEFAULT_STREAM_KEY = b"pyforge:wal"
...
PROCESSED_KEY_PREFIX = b"pyforge:processed:"
...
class WriteSyncTimeoutError(TimeoutError):
    ...
    set ``pyforge:processed:{msg_id}`` before ``timeout_ms`` elapses.
...
class WALProducer:
    ...
    - msgpack-list encoding of values in name_hash order (matches
      :func:`pyforge.schema.compile_schema` ordering).
    ...
    def write_sync(
        ...
        """``write`` plus a poll for the consumer's ``pyforge:processed:{id}`` key.
