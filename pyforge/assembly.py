...
def _compute_parallel_threshold() -> int:
    ...
    ``pyforge.assembly.PARALLEL_THRESHOLD`` accordingly (or just monkey-patch
...
#:
#: To override: ``pyforge.assembly.PARALLEL_THRESHOLD = N`` after import.
#: Recommended deployment: set ``NUMBA_NUM_THREADS=4`` in the environment
#: before importing ``pyforge`` for best batch perf across N.
...
def _assemble_batch_core(  # noqa: PLR0912, PLR0915
    ...
    The string-pool entry format mirrors :func:`pyforge.layout._read_string_bytes`:
    4-byte LE length at ``string_pool_data_offset + slot_id_offset``,
    followed by ``length`` UTF-8 bytes.

    Miss rows are zero-filled in-kernel so :class:`pyforge.pool.BatchBufferPool`'s
...
def assemble_batch(
    ...
    *,
    ...
    - If ``entity_ids[i]`` is found in the segment, ``out[i, :]`` holds its
      ...
      :func:`pyforge.serving.assemble`) and ``found_mask[i] == True``.
    ...
    Args:
        ...
        out: optional caller-supplied 2D buffer. Must be shape
            ...
            C-contiguous, writeable. Source from :class:`pyforge.pool.BatchBufferPool`
