"""Pyforge watchdog (Step 14).

Runs as a separate process from library producers/consumers. Reads the
``pyforge:heartbeats`` hash on a 30 s tick, detects PIDs whose
...
``pyforge:cleanup_queue`` (populated by ``SegmentRegistry.close`` when a
...
.. code-block:: bash
    ...
    python -m pyforge.watchdog --redis redis://localhost:6379/0
    python -m pyforge.watchdog --redis redis://localhost:6379/0 \\
...
:mod:`pyforge._internal.watchdog_lua` for the script + residual-risk
discussion.
...
from pyforge.shm import KEY_CLEANUP_QUEUE, _key_pid_segments
...
KEY_HEARTBEATS: Final[str] = "pyforge:heartbeats"
...
def _parse_heartbeat_value(value: bytes) -> tuple[int, int]:
    ...
    :data:`pyforge_watchdog_malformed_heartbeat_total`.
    """
...
class WatchdogState:
    ...
    def run_one_tick(self) -> TickResult:  # noqa: PLR0912, PLR0915 - linear orchestration
        ...
        4. Drain ``pyforge:cleanup_queue`` (up to ``batch_drain_size``).
        ...
        result = TickResult()
        try:
            # Step 1: HGETALL pyforge:heartbeats
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m pyforge.watchdog",
        description="Pyforge watchdog (Step 14): detects dead PIDs + cleans orphan segments.",
    )
    parser.add_argument(
        "--redis",
        required=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.metrics_port is not None:
        ...
        # ``import pyforge.watchdog`` (e.g. by tests, or by users
