...
Mirror image of Step 13's :mod:`pyforge._internal.insert_kernel` for the
...
import ``pyforge._internal.lookup_kernel`` (currently only
:mod:`pyforge.assembly`'s single-entity path) trigger LLVM/Numba init.

Why this exists
---------------
Pure-Python :func:`pyforge.layout.lookup` is ~2.91 µs hit / ~1.6 µs miss
...
:func:`pyforge._internal.hash_kernel.blake2b_8` instead of in the Python
...
- **All slot byte offsets passed as int64 scalar args** (invariant #15).
  Constants come from :data:`pyforge.layout.SLOT_DTYPE`'s field map at
  module load. A future ``SLOT_DTYPE`` reorder would fail loudly at
  ``pyforge.layout`` import (where the asserts live), not silently
...
- **Probe-loop shape mirrors :func:`pyforge.assembly._assemble_batch_core`'s
...
- **Pre-warm is opt-in.** Module load doesn't auto-compile;
  :func:`pyforge.assembly.prewarm` calls into :func:`prewarm` here so
...
from pyforge._internal.hash_kernel import blake2b_8
from pyforge._internal.hash_kernel import prewarm as _hash_prewarm
...
def _lookup_core(
    ...
    hit, ``-1`` on miss. Mirrors :func:`pyforge.layout.lookup`'s
    probe semantics exactly.

    BLAKE2b is computed inside the kernel via
    :func:`pyforge._internal.hash_kernel.blake2b_8` (Step 16c-d). Saves
    ...
    :func:`pyforge.layout._read_string_bytes`'s read pattern: ``uint32``
    ...
    :func:`pyforge.layout._insert_into_empty_slot` with ``flags`` written
...
def lookup_jit(segment: Segment, entity_id: str) -> int | None:
    """Numba-compiled equivalent of :func:`pyforge.layout.lookup`.
...
def prewarm() -> None:
    ...
    cheap. :func:`pyforge.assembly.prewarm` calls into here so a single
