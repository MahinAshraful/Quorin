"""Lua scripts shared by the real watchdog (:mod:`pyforge.watchdog`) and
the test simulator (:mod:`tests._watchdog_helpers`).

Two scripts:

* :data:`CLEANUP_DEAD_PID_LUA` — atomic dead-PID cleanup transaction.
  Walks ``pyforge:pid_segments:{pid}``, DECRs each segment's refcount,
...
* :data:`DRAIN_CLEANUP_QUEUE_LUA` — atomic drain. SPOPs up to
  ``ARGV[1]`` names from ``pyforge:cleanup_queue`` and returns them.
...
risk; alert on :data:`pyforge_watchdog_pid_reuse_abort_total` AND on
``pyforge:cleanup_queue`` size during incidents.
"""
...
-- KEYS:
--   1: pyforge:pid_segments:{pid}        (set of segment names held by dead pid)
--   2: pyforge:heartbeats                 (hash; for PID-reuse guard)
--   3: pyforge:cleanup_queue              (set of segments queued for posix_shm.unlink)
--   4: pyforge:segment_to_schema          (hash: segment_name -> schema_name)
-- ARGV:
...
for _, name in ipairs(segments) do
  local refcount_key = 'pyforge:refcount:' .. name
