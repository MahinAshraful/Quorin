...
Each library process that opens a Pyforge segment runs a single daemon thread
that ``HSET pyforge:heartbeats {pid} "{create_time_ns}:{wall_time_ns}"``
every :data:`HEARTBEAT_INTERVAL_SECONDS`. The watchdog (separate process,
:mod:`pyforge.watchdog`) reads this hash and detects dead PIDs via
consecutive missed advances of ``wall_time_ns``.

The thread mirrors :mod:`pyforge._internal.gc_manager`'s daemon-thread
...
from pyforge.metrics import heartbeat_age_seconds, heartbeat_writes_total
