...
2. Acquire ``pyforge:upgrade:lock:{safe_name}`` (NX EX 600s).
3. Verify the WAL stream is drained (XLEN == 0 AND XPENDING == 0) AND no
   live consumer (``pyforge:wal_consumer:liveness`` absent). The supported
   workflow demands this; the pause+reopen logic in
   :class:`pyforge.wal_consumer.WALConsumer` is a SAFETY NET, not a
   substitute. See ADR-014 §"Operator runbook".
4. Set ``pyforge:upgrade:pause:{safe_name}`` so any consumer that started
...
7. Per-field VECTORIZED translate from OLD bytes to NEW field arrays;
   build PyArrow table; call :func:`pyforge._internal.insert_kernel.insert_many`.
...
CRITICAL difference from hydrate's :func:`pyforge.hydration._force_drop_orphan`:
...
from pyforge._internal.evolution_lua import FLIP_SCHEMA_CURRENT_LUA, RELEASE_LOCK_LUA
from pyforge._internal.insert_kernel import insert_many
...
from pyforge.wal import DEFAULT_STREAM_KEY
from pyforge.wal_consumer import DEFAULT_GROUP_NAME, KEY_WAL_CONSUMER_LIVENESS
    ...
    *,
    ...
    Args:
        ...
        redis_client: Sync Redis client (the orchestrator is sync; mirrors
            :func:`pyforge.hydration.hydrate`).
    ...
    # pyforge:schema:{safe_name}:current convention; avoids collisions if a
    ...
    lock_key = f"pyforge:upgrade:lock:{safe_name}".encode()
    pause_key = f"pyforge:upgrade:pause:{safe_name}".encode()
    ...
    if consumer_alive:
        with contextlib.suppress(Exception):
            release_script(keys=[lock_key], args=[lock_token])
        raise UpgradeConflictError(
            "WAL consumer is still alive (pyforge:wal_consumer:liveness set) — "
    ...
    try:
        ...
        #    (UUID hex) so operators can `GET pyforge:upgrade:pause:{name}`
...
def _cleanup_orphan_new_segment(
    ...
    DIFFERENCE FROM :func:`pyforge.hydration._force_drop_orphan`:
...
# ---------------------------------------------------------------------------
# CLI: python -m pyforge.evolution upgrade ...
# ---------------------------------------------------------------------------
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m pyforge.evolution",
        description=(
            "Pyforge schema evolution (Step 15): atomic segment swap with "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.metrics_port is not None:
        ...
        from pyforge.metrics import registry as pyforge_registry  # noqa: PLC0415

        start_http_server(args.metrics_port, registry=pyforge_registry)
